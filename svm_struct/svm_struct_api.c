  ...
  if (fp == NULL) {
	...
	while(!feof(fp)) {
		...
		if(!examplesInitialised) {
			...
			examplesInitialised = true;
		}
		//currentExample = &examples[currentSentenceId];
	...
	//printf("File closed..\n");
	//for(int i = 0; i<n;i++) {
	//	printf("Framenumbers %i: y.N=%i, x.N=%i\n",i,examples[i].y.N,examples[i].x.N);
	//}
...
double loss = 0.0;
	for(int i = 0; i < y.N; i++) {
		if (i >= ybar.N)
			break;
		loss += y.seq[i] != ybar.seq[i] ? 1.0 : 0.0;
	}
	return loss;
}
